...
const seedTestOrders = async () => {
    try {
        ...
        let testUser = await user_model_1.User.findOne({ email: { $ne: null } });
        if (!testUser) {
            console.log("⚠️ No users found. Please create users first.");
            return;
        }
        // Get some products
        const products = await product_model_1.Product.find({ published: true }).limit(10);
...
exports.seedTestOrders = seedTestOrders;
